...
@lru_cache
def get_redis_client(): # Renamed for clarity, avoiding conflict with module name
    return aioredis.from_url(cfg.redis_url, decode_responses=True)

async def recall(user_id: int, topn: int):
    key = f"recall:{user_id}:{topn}"
    r = get_redis_client()
    
    cached_result = await r.get(key)
    if cached_result:
        # Data in Redis is already a JSON string of the list of dictionaries
    ...
    await r.set(key, json.dumps(results_list), ex=3600) # Store list of dicts
    return results_list